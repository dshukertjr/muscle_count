...
class App extends StatelessWidget {
  ...
  Widget build(context) {
    return MaterialApp(
      theme: ThemeData(primaryColor: Color(0xFFFFF579)),
            ...
            : StreamBuilder(
                ...
                builder:
                  ...
                  return Column(children: <Widget>[
                    Expanded(
                      child: workoutList.length == 0
                          ? FlareActor("assets/point.flr",
                              alignment: Alignment.center,
                              fit: BoxFit.contain,
                              animation: "below")
                          : ListView(children: workoutList),
                    ),
                    Container(
                ...
                builder:
                  ...
                  snaps.data.documents.forEach((snap) {
                    ...
                    final volume = double.parse(
                            workout["weight"] == "" ? "1" : workout["weight"]) *
                        double.parse(
                            workout["count"] == "" ? "1" : workout["count"]);
                    if (data.length > 0) {
                  ...
                  final chartdata = [
                    charts.Series<VolumePerDay, DateTime>(
                        id: 'Workout',
                        domainFn: (workout, _) => workout.time,
                        measureFn: (workout, _) => workout.volume,
                        data: data)
                  ];
                  return charts.TimeSeriesChart(chartdata);
                })));
  }
}
